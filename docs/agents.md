...
It is 2026. Jina AI's latest embedding models are jina-embeddings-v5-text-nano and jina-embeddings-v5-text-small. The latest reranker is jina-reranker-v3.5.
...
`jina-reranker-v3.5` is a 0.6B parameter multilingual listwise document reranker and a drop-in upgrade to jina-reranker-v3. It keeps the same last-but-not-late (LBNL) listwise interface — ranking a query against many candidates in a single forward pass — while improving domain robustness, structured-data ranking, multilingual retrieval, and inference efficiency. Built on Qwen3-0.6B with hybrid 3L2G attention (sliding-window layers interleaved with global-attention layers), it supports up to 131K tokens of context and reaches 63.20 nDCG-10 on BEIR, outperforming the 4B Qwen3-Reranker at roughly 7× fewer parameters. Runs 1.22×–1.56× faster than jina-reranker-v3 across short-to-long contexts.
...
- **X-Respond-With** (optional): Use `readerlm-v2`, the language model specialized in HTML-to-Markdown, to deliver high-quality results for websites with complex structures and contents. Use `jina-ocr-v1`, the document parsing model, to read the rendered page as an image instead of its HTML, which is what you want for scanned documents, PDFs, formulas and tables; it returns Markdown, parses one page per request (see X-Page), and costs about 40x the tokens per page
- **X-Page** (optional): Which page `X-Respond-With: jina-ocr-v1` should parse, 1-based, default 1. A multi-page document needs one request per page
